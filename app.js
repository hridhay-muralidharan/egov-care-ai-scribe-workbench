...
function statusLabel(status) {
  const labels = {
    not_started: "Ready For Input",
    in_discovery: "In Discovery",
    missing_evidence: "Evidence Needed",
    retry_needed: "Field Review",
    at_risk: "Safeguard Needed",
    blocked: "Governance Review",
    ready_for_review: "Ready For Review",
    approved: "Approved",
    pilot_candidate: "Evolution Loop"
  };
  return labels[status] || status.replaceAll("_", " ").replace(/\b\w/g, (char) => char.toUpperCase());
}
...
function routeTo(route) {
  const view = route === "demo" ? "workbench" : route || "product";
  ...
  els.navWorkbench.classList.toggle("active", view === "workbench");
...
function getReadiness() {
  if (appliedThrough < 0) return { score: 0, label: "Intake ready" };
...
function renderWorkbench() {
  ...
  els.confirmState.textContent = confirmed ? "Confirmed" : "Ready to confirm";
  els.confirmState.className = confirmed ? "pill success" : "pill warning";
  els.promptMeta.textContent = confirmed ? "Confirmed" : "Ready to confirm";
  els.promptText.textContent = step.prompt;

  els.confirmBtn.disabled = confirmed;
  els.confirmBtn.textContent = confirmed ? "Confirmed" : "Confirm input";
...
function openModal(step) {
  ...
  els.modalSummary.textContent = `Confirmed input updated ${Object.keys(step.updates).length} product-model workstreams and moved model confidence to ${step.readiness}%.`;
  els.modalInterpretation.textContent = step.response;
  els.knowMoreBtn.hidden = isFinalStep;
  els.knowMoreBtn.textContent = "Review evidence";
  els.continueBtn.textContent = isFinalStep ? "Open evidence board" : "Next input";
  ...
  Object.entries(step.updates).forEach(([name, update]) => {
    ...
    const copy = createElement("span");
    copy.appendChild(createElement("strong", "", name));
    copy.appendChild(createElement("span", "", update.note || "Updated based on the confirmed input."));
